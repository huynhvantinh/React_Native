...
function addOneBlog(dispatch){
    return ({tieuDe, noiDung, callBackResetInput, callbackGoToIndexScreen})=>{
        ...
        console.log("Vao function addOneBlog - CACH 11");
        callBackResetInput();
...
const removeOneBlog = (dispatch) => ({removed_id}) => {
        console.log("Vao function removeOneBlog - CACH 11");
        dispatch(actions.removeBlog({removed_id}));
    }
///////////// hết phần liệt viết những logic cần xử lý

console.log("Chay function createDataForContext");
const {Context, Provider} = createDataForContext({reducer,initialState, logicFunctions:{addOneBlog, removeOneBlog}})

export const BlogProviderUseReducer = ({children}) => {
    /**
     * CÁCH 11 - có dùng thêm createDataForContext - OK
